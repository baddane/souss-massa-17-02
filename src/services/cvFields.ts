// Extraction des champs d'un CV a partir de son texte brut : regex et
// dictionnaires, AUCUN LLM.
//
// Ce module est volontairement separe de `cvParser.ts` : il ne contient que de
// la logique pure, sans dependance au navigateur (pas de `File`, pas de pdf.js).
// Il peut donc etre appele aussi bien depuis l'interface admin que depuis un
// script Node de rattrapage (`scripts/parse-cvtheque.ts`), sans dupliquer les
// dictionnaires — une duplication finit toujours par diverger.
...
function findNiveau(t: string): { niveau: string; diplome: string } {
  ...
  // Libelle du diplome (premiere occurrence).
  // Le texte extrait d'un PDF arrive sur une seule ligne : sans borne, la
  // capture avalait la rubrique suivante (« Licence en gestion Competences:
  // Excel, Sage... »). On coupe donc aussi sur les intitules de rubrique.
  let diplome = '';
  const dm = t.match(/((?:licence|master|dipl[oô]me|ing[eé]nieur|dut|bts|baccalaur[eé]at|doctorat)[^\n.;,|]{0,60})/i);
  if (dm) {
    diplome = dm[1]
      .split(/\s(?=(?:comp[eé]tences?|langues?|exp[eé]riences?|formations?|contact|email|e-mail|t[eé]l)\b)/i)[0]
      .replace(/\s+/g, ' ')
      .trim();
  }
